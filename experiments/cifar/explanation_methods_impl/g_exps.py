...
for i, (img, seg_img) in enumerate(voc_ds):
    ...
    attrs_G = grad_exp.attribute(img.unsqueeze(0), target=label_idx).to(device)
    attrs.extend(attrs_G.to("cpu"))
    if len(attrs) % 1000 == 0:
        print(len(attrs), attrs[-1].shape, "  Time taken :", time.time() - start_time)
        torch.save(attrs, 'attrs/g_exp_{}.pt'.format(counter))
        attrs = []
        counter += 1
        start_time = time.time()
