...
resnet = resnet152(pretrained=True).to(device)
resnet = resnet.eval()
...
for i, (img, seg_img) in enumerate(voc_ds):
    ...
    attrs.extend(attrs_lime.to("cpu"))
    if len(attrs) % 1000 == 0:
        print(len(attrs), attrs[-1].shape, "  Time taken :", time.time() - start_time)
        torch.save(attrs, 'attrs/lime_exp_{}.pt'.format(counter))
        attrs = []
        counter += 1
        start_time = time.time()
...
torch.save(attrs, 'attrs2/lime_exp_{}.pt'.format(counter))
